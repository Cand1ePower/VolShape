...
backend/
  ...
  migrations/
    env.py
    versions/
      6d21ceff28f6_baseline_schema.py
```

`6d21ceff28f6_baseline_schema.py` 是当前 SQLAlchemy 模型的基线迁移，覆盖账号、订阅额度、New API token、LLM 请求审计、分层记忆、训练与饮食业务表。
...
这可以降低本地 MVP 调试成本，但生产和主开发库应以 Alembic 为 schema 版本来源。单靠 `create_all` 不适合生产演进，原因：
...
如果在本地没有 PostgreSQL，只想验证迁移脚本能否从空库跑通，可以临时覆盖数据库地址：

```bash
cd backend
DATABASE_URL=sqlite+aiosqlite:///alembic_upgrade_check.db alembic upgrade head
```
...
短期保留测试环境和本地 fallback 的 `create_all`，生产和开发主库使用 Alembic：

1. 新增/修改 SQLAlchemy 模型后，必须生成对应 migration。
2. review migration 脚本，确认索引、约束、默认值和数据迁移语义正确。
3. 在临时空库执行 `alembic upgrade head`。
4. 对已有环境升级前，先备份数据库。
5. 后续补 CI 检查 migration 是否可执行、模型和 migration 是否漂移。
...
- `alembic.ini` 只保留无密钥占位符，真实数据库地址从 `DATABASE_URL` 读取。
